...
SAVE_DIR = os.path.join(SCRIPT_DIR, "expert_data")
...
def interpolate_path(waypoints_df, total_time_steps, os_data=None):
    """웨이포인트(수정점)를 기반으로 부드러운 경로 생성 (속도 제약 고려)"""
    ...
    t_points = waypoints_df['time'].values
    ...
    # 속도 데이터가 있으면 제약 적용
    if os_data is not None and 'spd' in os_data.columns:
        # 각 웨이포인트 사이의 속도 확인 및 제약 적용
        constrained_x = x_points.copy()
        constrained_y = y_points.copy()
        
        for i in range(1, len(waypoints_df) - 1):
            prev_time = waypoints_df.iloc[i-1]['time']
            curr_time = waypoints_df.iloc[i]['time']
            next_time = waypoints_df.iloc[i+1]['time']
            
            # 해당 시간 구간의 평균 속도 찾기
            time_interval = curr_time - prev_time
            if time_interval > 0:
                # os_data에서 해당 시간의 속도 찾기
                time_idx = np.searchsorted(os_data['time'].values, curr_time)
                if time_idx < len(os_data):
                    speed = os_data.iloc[time_idx]['spd']
                    
                    # 가능한 범위 계산
                    prev_lon = constrained_x[i-1]
                    prev_lat = constrained_y[i-1]
                    curr_lon = constrained_x[i]
                    curr_lat = constrained_y[i]
                    next_lon = constrained_x[i+1] if i+1 < len(constrained_x) else None
                    next_lat = constrained_y[i+1] if i+1 < len(constrained_y) else None
                    
                    feasible_lons, feasible_lats, heading = calculate_feasible_waypoint_range(
                        prev_lon, prev_lat, curr_lon, curr_lat, next_lon, next_lat, speed, time_interval
                    )
                    
                    # 웨이포인트가 가능한 범위를 벗어나면 조정
                    if feasible_lons is not None:
                        # 현재 웨이포인트가 범위 내에 있는지 확인
                        min_lon, max_lon = min(feasible_lons), max(feasible_lons)
                        min_lat, max_lat = min(feasible_lats), max(feasible_lats)
                        
                        if curr_lon < min_lon or curr_lon > max_lon or curr_lat < min_lat or curr_lat > max_lat:
                            # 범위 내로 조정 (가장 가까운 점으로)
                            constrained_x[i] = max(min_lon, min(max_lon, curr_lon))
                            constrained_y[i] = max(min_lat, min(max_lat, curr_lat))
        
        x_points = constrained_x
        y_points = constrained_y
    
    # 3차 스플라인 보간
    kind = 'cubic' if len(waypoints_df) > 3 else 'linear'
    
    f_x = interp1d(t_points, x_points, kind=kind, fill_value="extrapolate")
    f_y = interp1d(t_points, y_points, kind=kind, fill_value="extrapolate")
    
    new_t = np.linspace(t_points[0], t_points[-1], total_time_steps)
    new_x = f_x(new_t)
    new_y = f_y(new_t)
...
if mode == "Expert Input Mode":
    ...
    # --- Interactive Path Editor using Folium Draw Plugin ---
    st.subheader("🗺️ Interactive Path Editor (Folium + KHOA ENC)")
    st.info("""
    **사용법:**
    1. 아래 지도에서 **오른쪽 상단 레이어 버튼**을 눌러 'KHOA Electronic Chart'를 켜세요.
    2. **왼쪽 그리기 도구(Polyline)**를 사용하여 새로운 경로를 그리거나 수정하세요.
    3. 다 그렸으면 **'✅ Apply Edited Path to Expert Path'** 버튼을 누르세요.
    """)
    ...
    if 'os_data' in st.session_state:
        ...
        if 'lat' in os_data.columns and 'lon' in os_data.columns:
            trajectory_lons = os_data['lon'].values
            trajectory_lats = os_data['lat'].values
        else:
            trajectory_lons = os_data['x'].values
            trajectory_lats = os_data['y'].values
        
        # Calculate map center and bounds
        center_lat = np.mean(trajectory_lats)
        center_lon = np.mean(trajectory_lons)
        
        # Create folium map
        m = folium.Map(
            location=[center_lat, center_lon],
            zoom_start=12,
            tiles='OpenStreetMap'  # 기본 베이스
        )

        # KHOA 전자해도 레이어 추가 (핵심)
        # 중요: 타일 주소 형식을 Folium에 맞게 설정
        selected_base_url = KHOA_TILE_BASE_URL_OPTIONS[2]
        khoa_tile_url = f"{selected_base_url}/ENC/{{z}}/{{x}}/{{y}}.png?key={KHOA_API_KEY}"
        
        folium.TileLayer(
            tiles=khoa_tile_url,
            attr='National Hydrographic and Oceanographic Agency',  # 저작권 표기 필수
            name='KHOA Electronic Chart (ENC)',
            overlay=True,  # OSM 위에 겹쳐서 표시
            control=True,
            show=True
        ).add_to(m)
        
        # Add original trajectory as polyline
        trajectory_coords = [[lat, lon] for lat, lon in zip(trajectory_lats, trajectory_lons)]
        folium.PolyLine(
            trajectory_coords,
            color='blue',
            weight=2,
            opacity=0.6,
            popup='Original OS Trajectory',
            tooltip='Click to see original path'
        ).add_to(m)
        
        # Add Draw plugin with edit and delete capabilities
        draw = Draw(
            export=True,
            position='topleft',
            draw_options={
        ...
        # Add markers for waypoints if they exist
        if 'waypoints' in st.session_state and len(st.session_state['waypoints']) > 0:
            waypoints = st.session_state['waypoints']
            if 'lat' in waypoints.columns and 'lon' in waypoints.columns:
                wp_lons = waypoints['lon'].values
                wp_lats = waypoints['lat'].values
            else:
                wp_lons = waypoints['x'].values
                wp_lats = waypoints['y'].values
            
            for i, (lat, lon) in enumerate(zip(wp_lats, wp_lons)):
                folium.CircleMarker(
                    location=[lat, lon],
                    radius=5,
                    color='yellow',
        ...
        # Display map and get user interactions
        map_output = st_folium(
            m,
            width=1200,
            height=600,
            returned_objects=["all_drawings", "last_active_drawing", "last_draw"]
        )
        
        # Process drawn/edited path
        col_apply, col_clear_folium, col_info = st.columns([2, 2, 3])
        
        with col_apply:
            if st.button("✅ Apply Edited Path to Expert Path", key="apply_folium_path", use_container_width=True):
                if map_output and map_output.get('all_drawings'):
                    # Parse the drawn path
                    edited_path_df = parse_folium_draw_output(map_output, os_data)
                    
                    if edited_path_df is not None and len(edited_path_df) > 0:
                        # Update expert path with edited trajectory
                        st.session_state['drawn_path'] = edited_path_df
                        st.session_state['use_drawn_path'] = True
                        st.session_state['use_original_path'] = False
                        
                        # Also update waypoints based on edited path
                        # Extract key waypoints from edited path (e.g., every Nth point or based on curvature)
                        num_waypoints = min(max(3, len(edited_path_df) // 20), 10)
                        waypoints_from_edited = initialize_waypoints_from_trajectory(edited_path_df, num_waypoints=num_waypoints)
                        st.session_state['waypoints'] = waypoints_from_edited
                        
                        st.success(f"✅ Path updated! {len(edited_path_df)} trajectory points applied from map editing.")
                        st.rerun()
                    else:
                        st.warning("⚠️ No valid path data found. Please draw a polyline on the map first.")
                else:
                    st.warning("⚠️ No path has been drawn or edited on the map. Use the drawing tools (top-left) to create or modify a path.")
        
        with col_clear_folium:
            if st.button("🗑️ Clear All Drawings", key="clear_folium_drawings", use_container_width=True):
                # Clear the folium drawings from session state
                # Note: This doesn't clear the map itself, but clears our reference
                if 'folium_last_drawings' in st.session_state:
                    del st.session_state['folium_last_drawings']
                st.info("💡 Refresh the page to fully reset the map, or use the delete tool on the map to remove individual drawings.")
        
        with col_info:
            if map_output and map_output.get('all_drawings'):
                num_drawings = len(map_output['all_drawings'])
                st.info(f"📊 **Status:** {num_drawings} drawing(s) on map. Click 'Apply Edited Path' to use them.")
            else:
                st.info("💡 **Tip:** Use the drawing tools in the top-left corner of the map to draw polylines or markers.")
    
    st.divider()

    with col2:
        st.header("🖊️ Edit OS Path")
        st.info("Modify the expert path (OS) by adjusting control points. This will be used for Inverse Reinforcement Learning.")
        
        # Control Point 이동 방법 안내
        with st.expander("📖 How to Move Control Points", expanded=True):
            st.markdown("""
            **Map Click Method:**
            1. **Click** a yellow Control Point (WP0, WP1, ...) on the map to select it (turns RED)
            2. **Click** anywhere on the map (trajectory lines work best) to see coordinates
            3. A **green marker** will appear at the clicked location with coordinates displayed
            4. **Edit** the coordinates if needed using the number inputs
            5. **Click** "Move CP Here" button to move the selected Control Point to that location
            6. **Click** the same Control Point again to deselect
            
            **Tip:** 
            - Control Points are displayed as yellow circles. Selected points turn red and become larger.
            - Clicking on trajectory lines (OS or TS) provides the most reliable coordinate detection.
            - You can manually edit coordinates before moving the Control Point.
            """)
        
        # Control Point 관리 섹션
        st.subheader("Control Point Management")
        
        waypoints = st.session_state['waypoints'].reset_index(drop=True)
        ...
        with col_add:
            if st.button("➕ Add Point", key="add_point"):
                # 현재 항적에서 중간 지점에 새 포인트 추가
                if len(waypoints) > 0:
                    # 마지막 포인트와 그 이전 포인트 사이에 추가
                    if len(waypoints) >= 2:
                        last_idx = len(waypoints) - 1
                        prev_idx = last_idx - 1
                        # 시간 중간값 계산
                        mid_time = (waypoints.iloc[prev_idx]['time'] + waypoints.iloc[last_idx]['time']) / 2
                        # 보간된 경로에서 해당 시간의 위치 찾기
                        os_data = st.session_state['os_data']
                        expert_path = interpolate_path(waypoints, len(os_data), os_data=os_data)
                        time_idx = np.searchsorted(expert_path['time'].values, mid_time)
                        time_idx = min(time_idx, len(expert_path) - 1)
                        
                        new_point = expert_path.iloc[time_idx].copy()
                        new_point['time'] = mid_time
                        
                        # 새 포인트를 waypoints에 추가 (시간 순서대로)
                        new_waypoints = pd.concat([waypoints, pd.DataFrame([new_point])], ignore_index=True)
                        new_waypoints = new_waypoints.sort_values('time').reset_index(drop=True)
                        st.session_state['waypoints'] = new_waypoints
                        st.session_state['use_original_path'] = False  # Control Point 추가 시 보간된 경로 사용
                        st.success(f"✅ Control Point added (Total: {len(new_waypoints)})")
                        st.rerun()
                    else:
                        st.warning("Need at least 2 points to add a new one")
        
        with col_remove:
            if st.button("➖ Remove Point", key="remove_point", disabled=(num_waypoints <= 2)):
                if num_waypoints > 2:
                    # 마지막 포인트 제거 (첫 번째와 마지막은 유지)
                    new_waypoints = waypoints.iloc[:-1].copy() if len(waypoints) > 2 else waypoints.copy()
                    st.session_state['waypoints'] = new_waypoints
                    st.session_state['use_original_path'] = False  # Control Point 삭제 시 보간된 경로 사용
                    st.success(f"✅ Control Point removed (Total: {len(new_waypoints)})")
                    st.rerun()
                else:
                    st.warning("Need at least 2 control points")
        
        # Control Point 재배치 버튼
        if st.button("🔄 Redistribute Points", key="redistribute_points"):
            # 현재 보간된 경로에 따라 포인트 재배치
            os_data = st.session_state['os_data']
            expert_path = interpolate_path(waypoints, len(os_data), os_data=os_data)
            num_points = len(waypoints)
            redistributed = redistribute_waypoints_on_path(waypoints, expert_path, num_waypoints=num_points)
            st.session_state['waypoints'] = redistributed
            st.session_state['use_original_path'] = False  # 재배치 시 보간된 경로 사용
            st.success("✅ Control Points redistributed along the current path")
            st.rerun()
        
        st.markdown("---")
        
        # Control Point 직접 편집 테이블
        st.subheader("✏️ Direct Control Point Editor")
        st.info("Edit Control Point coordinates directly in the table below. Changes will be applied when you click 'Apply Changes'.")
        
        # Control Point 편집용 데이터프레임 생성
        if 'waypoints' in st.session_state and len(st.session_state['waypoints']) > 0:
            wp_df = st.session_state['waypoints'].copy()
            
            # 편집 가능한 컬럼만 선택
            edit_cols = ['time']
            if 'lon' in wp_df.columns:
                edit_cols.append('lon')
            if 'lat' in wp_df.columns:
                edit_cols.append('lat')
            if 'x' in wp_df.columns and 'lon' not in wp_df.columns:
                edit_cols.append('x')
            if 'y' in wp_df.columns and 'lat' not in wp_df.columns:
                edit_cols.append('y')
            
            wp_edit_df = wp_df[edit_cols].copy()
            wp_edit_df.insert(0, 'CP_ID', [f'WP{i}' for i in range(len(wp_edit_df))])
            
            # 편집 가능한 데이터프레임 표시
            edited_df = st.data_editor(
                wp_edit_df,
                column_config={
                    "CP_ID": st.column_config.TextColumn("Control Point", disabled=True),
                    ...
                    "x": st.column_config.NumberColumn("X (Lon)", format="%.6f"),
                    "y": st.column_config.NumberColumn("Y (Lat)", format="%.6f"),
                },
                use_container_width=True,
                num_rows="fixed",
                key="cp_editor"
            ...
            with col_apply:
                if st.button("✅ Apply Changes", key="apply_cp_changes", use_container_width=True):
                    # 편집된 데이터를 waypoints에 반영
                    updated_waypoints = st.session_state['waypoints'].copy()
                    
                    for idx, row in edited_df.iterrows():
                        # CP_ID에서 인덱스 추출
                        cp_id = row['CP_ID']
                        cp_idx = int(cp_id.replace('WP', ''))
                        
                        if cp_idx < len(updated_waypoints):
                            # 시간 업데이트
                            if 'time' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('time')] = row['time']
                            
                            # 좌표 업데이트
                            if 'lon' in row and 'lon' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('lon')] = row['lon']
                            if 'lat' in row and 'lat' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('lat')] = row['lat']
                            if 'x' in row and 'x' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('x')] = row['x']
                            if 'y' in row and 'y' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('y')] = row['y']
                            
                            # lon/lat과 x/y 동기화
                            if 'lon' in updated_waypoints.columns and 'x' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('x')] = updated_waypoints.iloc[cp_idx]['lon']
                            if 'lat' in updated_waypoints.columns and 'y' in updated_waypoints.columns:
                                updated_waypoints.iloc[cp_idx, updated_waypoints.columns.get_loc('y')] = updated_waypoints.iloc[cp_idx]['lat']
                    ...
                    # 시간 순으로 정렬
                    updated_waypoints = updated_waypoints.sort_values('time').reset_index(drop=True)
                    st.session_state['waypoints'] = updated_waypoints
                    st.session_state['use_original_path'] = False
                    
                    st.success("✅ Control Points updated successfully!")
                    st.rerun()
            
            with col_reset:
                if st.button("🔄 Reset to Current", key="reset_cp_editor", use_container_width=True):
                    st.rerun()
        
        st.markdown("---")
        
        # 원본 vs 수정된 항적 비교
        st.subheader("📊 Original vs Modified Trajectory Comparison")
        
        if st.checkbox("Show Comparison", key="show_comparison", value=False):
            os_data = st.session_state['os_data']
            ...
            # 원본 항적
            os_lon = os_data['lon'] if 'lon' in os_data.columns else os_data['x']
            os_lat = os_data['lat'] if 'lat' in os_data.columns else os_data['y']
            
            fig_compare.add_trace(go.Scattermapbox(
                lon=os_lon,
                lat=os_lat,
                mode='lines',
            ...
            fig_compare.add_trace(go.Scattermapbox(
                ...
                lat=expert_lat,
                mode='lines',
                name='Modified Trajectory (Expert Path)',
                line=dict(color='blue', width=3),
                showlegend=True,
                hovertemplate='<b>Expert Path</b><br>Lat: %{lat:.6f}<br>Lon: %{lon:.6f}<extra></extra>'
            ))
            
            # 지도 배경 설정
            center_lon = np.mean([os_lon.mean(), expert_lon.mean()])
            center_lat = np.mean([os_lat.mean(), expert_lat.mean()])
            
            map_bg = st.session_state.get('map_background', 'OpenStreetMap Only')
            base_style = "open-street-map"
            ...
            with col_stat1:
                # 평균 거리 차이 계산
                if len(os_data) == len(expert_path):
                    distances = np.sqrt((os_lon.values - expert_lon.values)**2 + (os_lat.values - expert_lat.values)**2)
                    # 위도/경도 차이를 미터로 변환 (대략적)
                    distances_m = distances * 111000  # 1도 ≈ 111km
                    avg_distance = np.mean(distances_m)
                    st.metric("Average Deviation", f"{avg_distance:.1f} m")
            
            with col_stat2:
                cp_mod_count = len(st.session_state.get('cp_modification_history', []))
                st.metric("Control Points Modified", f"{cp_mod_count}")
            
            with col_stat3:
                path_type = "Drawn Path" if use_drawn_path else ("Original" if use_original else "Interpolated")
                st.metric("Path Type", path_type)
        ...
        if len(cp_history) > 0:
            # 최근 수정 이력 표시
            history_df = pd.DataFrame(cp_history)
            st.dataframe(history_df, use_container_width=True, height=200)
            
            col_undo, col_clear = st.columns(2)
            with col_undo:
                if st.button("↩️ Undo Last Modification", key="undo_last_mod", use_container_width=True):
                    if len(cp_history) > 0:
                        last_mod = cp_history[-1]
                        cp_idx = last_mod['cp_id']
                        
                        # waypoints 복원
                        waypoints_df = st.session_state['waypoints'].copy()
                        if 'lon' in waypoints_df.columns:
                            waypoints_df.iloc[cp_idx, waypoints_df.columns.get_loc('lon')] = last_mod['old_lon']
                        if 'lat' in waypoints_df.columns:
                            waypoints_df.iloc[cp_idx, waypoints_df.columns.get_loc('lat')] = last_mod['old_lat']
                        if 'x' in waypoints_df.columns:
                            waypoints_df.iloc[cp_idx, waypoints_df.columns.get_loc('x')] = last_mod['old_lon']
                        if 'y' in waypoints_df.columns:
                            waypoints_df.iloc[cp_idx, waypoints_df.columns.get_loc('y')] = last_mod['old_lat']
                        
                        st.session_state['waypoints'] = waypoints_df
                        st.session_state['cp_modification_history'] = cp_history[:-1]
                        
                        st.success(f"✅ Undone modification of Control Point {cp_idx}")
                        st.rerun()
            
            with col_clear:
                if st.button("🗑️ Clear All History", key="clear_history", use_container_width=True):
                    st.session_state['cp_modification_history'] = []
                    st.success("✅ Modification history cleared")
                    st.rerun()
        else:
        ...
        # 3D 검증 화면 버튼
        st.subheader("🔍 3D Verification View")
        if st.button("📊 Show 3D Space-Time View", key="show_3d_view"):
            st.session_state['show_3d_view'] = True
            st.rerun()
        
        # 3D 시각화 표시
        if st.session_state.get('show_3d_view', False):
            st.markdown("### 3D Space-Time Cube Visualization")
            
            # 3D 그래프 생성
            fig_3d = go.Figure()
            
            # OS Expert Path (3D)
            expert_path = interpolate_path(st.session_state['waypoints'], len(st.session_state['os_data']), 
                                         os_data=st.session_state['os_data'])
            expert_lon = expert_path['lon'] if 'lon' in expert_path.columns else expert_path['x']
            expert_lat = expert_path['lat'] if 'lat' in expert_path.columns else expert_path['y']
            expert_time = expert_path['time'].values
            ...
            # CP 수정 이력 정보 포함
            cp_history = st.session_state.get('cp_modification_history', [])
            expert_3d_customdata = []
            for i in range(len(expert_path)):
                time_val = expert_path.iloc[i]['time']
                cp_mods = [m for m in cp_history if abs(m['time'] - time_val) < 1.0]
                cp_info = f"CP modified: {len(cp_mods)}" if cp_mods else "Original path"
                expert_3d_customdata.append([
                    'OS (Expert Path)',
                    time_val,
                    cp_info
                ])
            ...
            fig_3d.add_trace(go.Scatter3d(
                ...
                marker=dict(size=3, color=expert_time, colorscale='Viridis', showscale=True, colorbar=dict(title="Time (s)")),
                customdata=expert_3d_customdata,
                hovertemplate='<b>%{customdata[0]}</b><br>Time: %{customdata[1]:.1f}s<br>%{customdata[2]}<br>Lon: %{x:.6f}<br>Lat: %{y:.6f}<extra></extra>'
            ))
            ...
            for idx, ts in enumerate(st.session_state['ts_list']):
                ...
                fig_3d.add_trace(go.Scatter3d(
                    ...
                    marker=dict(size=2, color=ts_time, colorscale='Reds', showscale=False),
                    hovertemplate=f'<b>TS{idx+1}</b><br>Time: %{{z:.1f}}s<br>Lon: %{{x:.6f}}<br>Lat: %{{y:.6f}}<extra></extra>'
                ))
            
            # Control Points (3D)
            waypoints = st.session_state['waypoints']
            if len(waypoints) > 0:
                wp_lon = waypoints['lon'].values if 'lon' in waypoints.columns else waypoints['x'].values
                wp_lat = waypoints['lat'].values if 'lat' in waypoints.columns else waypoints['y'].values
                wp_time = waypoints['time'].values
                
                # CP 수정 이력 정보
                cp_mod_info = []
                for i, t in enumerate(wp_time):
                    mods = [m for m in cp_history if m['cp_id'] == i]
                    info = f"Modified {len(mods)} time(s)" if mods else "Original"
                    cp_mod_info.append(info)
                ...
                fig_3d.add_trace(go.Scatter3d(
                    ...
                    text=[f"WP{i}" for i in range(len(waypoints))],
                    textposition="top center",
                    customdata=cp_mod_info,
                    hovertemplate='<b>Control Point %{text}</b><br>Time: %{z:.1f}s<br>%{customdata}<br>Lon: %{x:.6f}<br>Lat: %{y:.6f}<extra></extra>'
                ))
            ...
            fig_3d.update_layout(
                ...
                height=700,
                title="3D Space-Time Cube: Expert Behavior History"
            )
            
            st.plotly_chart(fig_3d, width='stretch', key="3d_view_chart")
            
            if st.button("❌ Close 3D View", key="close_3d_view"):
    ...
    **사용법:**
    ...
    if not files:
        ...
        if selected_file:
            ...
            # 데이터 다운로드 버튼
            with open(os.path.join(SAVE_DIR, selected_file), "rb") as f:
>>>>>>> e0cf47f25ba8d2f2e0fbf991b540944008788164
